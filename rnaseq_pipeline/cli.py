...
from rnaseq_pipeline.tasks import SubmitExperimentToGemma, SubmitExperimentsFromGoogleSpreadsheetToGemma, SubmitExperimentBatchInfoToGemma
...
def submit_experiment(argv):
    ...
    run_luigi_task(SubmitExperimentToGemma(experiment_id=args.experiment_id, rerun=args.rerun, priority=args.priority), args)
...
def submit_experiment_batch_info(argv):
    ...
    run_luigi_task(SubmitExperimentBatchInfoToGemma(experiment_id=args.experiment_id, ignored_samples=args.ignored_samples, rerun=args.rerun), args)
...
def submit_experiments_from_gsheet(argv):
    ...
    parser.add_argument('--local-scheduler', action='store_true', default=False)
    args = parser.parse_args(argv)
    run_luigi_task(SubmitExperimentsFromGoogleSpreadsheetToGemma(args.spreadsheet_id, args.sheet_name, ignore_priority=args.ignore_priority), args)
...
def main():
    if len(sys.argv) < 2:
        ...
        sys.exit(1)
    command = sys.argv[1]
    if command == 'submit-experiment':
        sys.exit(submit_experiment(sys.argv[2:]))
    elif command == 'submit-experiment-batch-info':
        sys.exit(submit_experiment_batch_info(sys.argv[2:]))
    elif command == 'submit-experiments-from-gsheet':
        sys.exit(submit_experiments_from_gsheet(sys.argv[2:]))
    else:
        print(f'Unknown command {command}. Possible values are: submit-experiment, submit-experiment-batch-info, submit-experiments-from-gsheet.')
        sys.exit(1)
